...
export const SearchedProducts = ({searchedProduct}) => {
    ...
    return (
        ...
        <div className={`grid gap-2 ${
            SearchedProducts.length === 0 ? ("grid-cols-1 ") :
            SearchedProducts.length === 1 ? ("grid-cols-2 ") :
            SearchedProducts.length === 2 ? ("grid-cols-2 lg:grid-cols-3") :
            SearchedProducts.length >= 3 ? ("grid-cols-2 sm:grid-cols-3 lg:grid-cols-4"):
              null }  `}>
          {searchedProduct?.map((product) => (
               <Card key={product.id} className="w-full max-w-sm transform-all duration-300 hover:scale-102 ease-in-out bg-gray-500 border-none text-white">
              <CardHeader>
